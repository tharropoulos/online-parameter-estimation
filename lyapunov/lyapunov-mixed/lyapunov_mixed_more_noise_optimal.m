...
mkdir(fullfile('figures','more-noise', 'optimal', 'ab', 'eps'));
mkdir(fullfile('figures','more-noise', 'optimal', 'xdif', 'svg'));
mkdir(fullfile('figures','more-noise', 'optimal', 'xdif', 'eps'));
...
for i = 1:length(foptions)
    for j = 1:length(n0options)
        ...
        [data, odex] = lyapunov_mixed(tspan, initialconditions, a, b, gamma1, gamma2, theta_m, u, n);
        ...
        title(sprintf('Approximated and actual output using the Lyapunov Mixed  Method for $n(t) = %.1fsin(2\\pi %d t)$', n0options(j), foptions(i)), 'Interpreter', 'latex');
%         ax = gca;
%         ax.TitleFontSizeMultiplier = 0.9;
        ...
        title(sprintf('Approximated and actual parameters using the Lyapunov Mixed Method for $n(t) = %.1fsin(2\\pi %d t)$', n0options(j), foptions(i)), 'Interpreter', 'latex');
%         ax = gca;
%         ax.TitleFontSizeMultiplier = 0.9;
        ...
        yline(0, '--');
%         ylim([-0.5, 4]);
       title(sprintf('Difference between actual and approximated output using the Lyapunov Mixed Method for  $n(t) = %.1fsin(2\\pi %d t)$', n0options(j), foptions(i)), 'Interpreter', 'latex');
        ylabel('$x - \hat{x}$', 'interpreter', 'latex');
        xlabel('$t$', 'interpreter', 'latex');
    ...
    saveas(fig2, fullfile('figures',  'more-noise', 'optimal', 'ab', 'svg', sprintf('ab_f(%.1f).svg', foptions(i))));
    saveas(fig2, fullfile('figures',  'more-noise', 'optimal', 'ab', 'eps', sprintf('ab_f(%.1f).eps', foptions(i))));
    
    saveas(fig3, fullfile('figures',  'more-noise', 'optimal', 'xdif', 'svg', sprintf('xdif_f(%.1f).svg', foptions(i))));
